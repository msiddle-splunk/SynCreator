...
def rum():
    if g.user is None:
        ...
        db.execute(
            "UPDATE scc"
            " SET realm = ?, rum_token = ?, ingest_token = ? WHERE username = ?",
...
@bp.route("/view/<string:username>/quiz_pass.html", methods=["GET"])
def quiz_pass(username):
    db = get_db()
    settings = db.execute(
    ...
    return render_template("quiz_pass.html", settings=settings)


@bp.route("/view/<string:username>/quiz_fail.html", methods=["GET"])
def quiz_fail(username):
    db = get_db()
    settings = db.execute(
        "SELECT condition, realm, rum_token, ingest_token, username as u"
        " FROM scc WHERE username = ?",
    ...
    return render_template("quiz_fail.html", settings=settings)
...
@bp.route("/view/<string:username>/cart/complete", methods=["POST"])
def complete(username):
    db = get_db()
    settings = db.execute(
        "SELECT condition, realm, rum_token, ingest_token, username as u, hec_url, hec_token"
        " FROM scc WHERE username = ?",
        (username,),
    ).fetchone()
...
def cart(username):
    productid = request.args.get("productid", "default")
    db = get_db()
    settings = db.execute(
        "SELECT condition, realm, rum_token, ingest_token, username as u"
        " FROM scc WHERE username = ?",
    ...
    return render_template("cart/index.html", settings=settings, productid=productid)
...
def create_browser_test():
    if g.user is None:
        ...
        document_path = (
            ...
            + "/synthetic_tests/homepage-us.json"
        )
        ...
        with open(document_path, "r") as f:
            ...
            data["test"][
                "name"
            ] = f"[syncreator - {g.user['username']}] Home - Desktop {g.user['realm']}"
            data["test"]["transactions"][0]["steps"][0]["url"] = (
                "https://splunko11y.com/syncreator/view/" + g.user["username"]
            )
            data["test"]["transactions"][0]["steps"][0]["options"]["url"] = (
                "https://splunko11y.com/syncreator/view/" + g.user["username"]
            )
